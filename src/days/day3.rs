...
impl RegexDay3 {
    fn parsery(&self, input: &str) -> Vec<(i128, i128)> {
        let regexed = regex::Regex::new(r"mul\((\d*),(\d*)\)").unwrap();
        let result = regexed
            .captures_iter(input)
            .map(|i| {
                let a = i.get(1).unwrap().as_str().parse::<i128>().unwrap();
                let b = i.get(2).unwrap().as_str().parse::<i128>().unwrap();
                (a, b)
            })
    ...
    fn do_dont_parsery(&self, input: &str) -> Vec<(Option<bool>, i128, i128)> {
        ...
        let result = regexed
            ...
            .map(|i| {
                ...
                let b = i.get(2).map_or("0", |i| i.as_str());
                let c = i.get(3).map_or("0", |i| i.as_str());
                match a {
                    "do" => (
                        Some(true),
                        b.parse::<i128>().unwrap(),
                    ...
                    "mul" => (None, b.parse::<i128>().unwrap(), c.parse::<i128>().unwrap()),
                    _ => (None, 0, 0),
                }
            })
            .collect();
